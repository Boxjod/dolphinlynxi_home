...
// 数据集市场页（Marketplace）
// 功能：多维度筛选 + 搜索 + 卡片列表
// 数据来源：lib/data.ts → datasets 数组
// 文案：i18n.ts market.* / common.* 键
// ============================================================
'use client';
...
// 供应方 → CSS class 后缀（精选=b / 众包=crowd / 海豚自营=空字符串）
const vendorCls = (v: string) => v === '精选合集' ? 'b' : v === '众包采集' ? 'crowd' : '';

// 帧数格式化：>1亿 → "x.xx亿"，>1万 → "x.x万"，否则原始逗号分隔
...
export default function MarketplacePage() {
  ...
  // ── 筛选状态 ──────────────────────────────────────────────
  const [scene, setScene] = useState('全部');   // 当前选中的场景
  const [arm, setArm] = useState('全部');       // 当前选中的机械臂类型
  const [vendor, setVendor] = useState('全部'); // 当前选中的供应方
  const [search, setSearch] = useState('');     // 搜索框文本

  // ── 过滤结果（依赖 scene/arm/vendor/search 变化时重算）──
