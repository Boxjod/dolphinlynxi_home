...
(function () {
  ...
  function enforceHomeChineseEarly() {
  ...
  function init() {
    // Home page must always open in Chinese by default.
    enforceHomeChineseEarly();

    document.documentElement.setAttribute('lang', state.lang === 'zh' ? 'zh-CN' : 'en');
    document.documentElement.setAttribute('data-lang', state.lang);
    const homeZhDefault = isHomePath() && state.lang === 'zh';

    // Home uses Chinese baseline content; avoid mutating DOM before hydration.
    if (homeZhDefault) {
      ...
      if (document.readyState === 'complete') {
        ...
        window.addEventListener('load', runPostHydration, { once: true });
      }
      return;
    }
  ...
  // Lock homepage language as early as possible to avoid first-frame language flicker.
  enforceHomeChineseEarly();
